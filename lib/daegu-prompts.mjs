// lib/daegu-prompts.js — 대구 소식 프롬프트 빌더 (순수 함수, SDK 의존성 없음)
//
// 서버(lib/ai.js, OpenAI 폴백)와 로컬 스크립트(scripts/daegu-write.mjs, claude -p 구독)가
// 같은 프롬프트를 쓰도록 여기 한 곳에만 둔다. Next 전용 import를 넣지 말 것 —
// 이 파일은 plain node(ESM)에서도 그대로 import 된다.

export function buildWritePrompt({ candidates, today, recentTitles = [] }) {
...
아래 후보 기사/포스트 목록에서 대구 시민·방문객에게 가장 유용한 주제 1개를 골라
정보글을 한국어로 작성해라.

주제 우선순위 (위에서부터):
1. **오늘~며칠 내 개최되거나 지금 진행 중인 축제·행사** — 기사 날짜가 며칠 지났어도 행사가 임박했으면 최우선
2. **대구 생활에 영향이 큰 지역 이슈** — 지역 경제·기업 소식(인수·개점·폐점 등), 폭염·호우 등 날씨, 교통·정책 변화
3. 시민 안전에 유용한 사건사고 소식
4. 앞으로 열릴 예정인 축제·행사·전시
5. 핫플레이스·트렌드·재미있는 지역 소식

톤 규칙:
- 축제·행사·핫플은 밝고 친근한 톤.
- 사건사고·재난·날씨 경보는 차분한 정보 전달 톤 — 이모지 금지, 선정적 표현 금지,
  피해자의 신상·상태 묘사 금지, 시민이 알아둘 정보(주의사항·우회로·대처법 등) 중심.
...
export function buildReviewPrompt({ post, candidates, today }) {
...
질문 1~3은 **초안의 주제가 축제·행사·공연처럼 기간이 있는 행사일 때만** 해당한다.
사건사고·경제(인수·개점 등)·날씨·정책 같은 일반 뉴스 주제면 질문 1~3은 무조건 null로 답해라
(이미 일어난 일을 전하는 것이 뉴스의 본질이므로 "끝났다"고 판정하지 마라).
...
// 모델 응답 텍스트에서 JSON 객체만 추출 — 코드펜스·잡담이 섞여도 견디게
...
// 작성 응답 정규화 — 백엔드(OpenAI·claude -p) 공통
export function normalizeDraft(obj) {
  if (!obj) return { error: "JSON 없음" };
  if (obj.skip) return { skip: true, reason: String(obj.reason || "조건 만족 주제 없음") };
  if (!obj.title || !Array.isArray(obj.sections) || !obj.sections.length) {
    return { error: "필수 필드 누락(title/sections)" };
  }
  return {
    ...
    tags: Array.isArray(obj.tags)
      ? obj.tags.map((t) => String(t).trim()).filter(Boolean).slice(0, 6)
      : [],
    used_links: Array.isArray(obj.used_links) ? obj.used_links.filter(Boolean) : [],
  };
}

// 검수 응답 → 판정. 판정은 모델이 아니라 코드가 한다(기존 동작 유지).
export function judgeReview(obj, today) {
  if (!obj) return { approved: true, issues: [] };
  const issues = [];
  let ended = false;
  ...
  return { approved: issues.length === 0, issues, ended };
}

...
밝고 친근한 톤의 정보글을 한국어로 작성해라. 앞으로 열리거나 지금 진행 중인 축제·행사·핫플·트렌드 우선.
...
export async function aiReviewDaeguPost({ post, candidates, today }) {
  ...
  const prompt = `
너는 팩트체커야. 오늘은 ${today}.
아래 초안의 사실 주장(날짜·장소·행사 진행 여부·프로그램)이 근거 목록으로 뒷받침되는지 검사해라.

거부해야 하는 경우 (이 세 가지만):
1. 근거에 행사가 **이미 종료됐다는 표시**("마쳤다", "성황리에 폐막", 종료일이 오늘 이전)가 있는데
   초안이 진행 중/예정처럼 서술
2. 근거에 전혀 없는 구체적 날짜·장소·가격을 초안이 **단정적으로** 서술
3. 근거가 전부 칼럼/회고인데 초안이 현재 소식처럼 포장

거부하면 안 되는 경우 (중요):
- "예매 시작", "개최 예정", "개막 앞둬" 같은 소식은 **앞으로 열리는 행사라는 뜻이므로 좋은 주제다. 승인하라.**
- 초안이 세부 일정을 단정하지 않고 "공식 안내 확인" 식으로 유보했다면, 근거에 세부 일정이 없어도 문제 삼지 마라.
- 종료 여부를 추측으로 판단하지 마라. 근거에 종료 표시가 없으면 종료로 간주하지 않는다.

출력은 JSON만: {"approved":true} 또는 {"approved":false,"issues":["문제1","문제2"]}

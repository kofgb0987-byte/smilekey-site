...
export async function aiReviewDaeguPost({ post, candidates, today }) {
  ...
  const prompt = `
너는 팩트체커야. 오늘은 ${today}. 아래 두 가지 질문에만 답해라. 승인/거부 판단은 하지 마라.
...
질문 2 — unsupported_claims: 초안이 **단정적으로** 서술한 구체적 사실(날짜·장소·가격·프로그램) 중
근거 목록에 없는 것을 나열하라. 초안이 "공식 안내 확인", "~로 알려져 있어요"처럼
유보한 항목과, 상식 수준의 일반 서술은 포함하지 마라. 없으면 빈 배열.

출력은 JSON만:
{"ended_evidence": "인용문" 또는 null, "unsupported_claims": ["주장1", ...]}
...
`.trim();
  ...
  try {
    ...
    // 판정은 코드가: 종료 증거가 인용됐거나, 근거 없는 단정 주장이 있을 때만 거부
    const issues = [];

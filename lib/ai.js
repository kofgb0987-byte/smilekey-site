...
export async function aiReviewDaeguPost({ post, candidates, today }) {
  ...
  const prompt = `
너는 깐깐한 팩트체커야. 오늘은 ${today}.
아래 초안의 모든 사실 주장(날짜·장소·행사 진행 여부·프로그램)이 근거 목록으로 뒷받침되는지 검사해라.

반드시 거부해야 하는 경우:
- 이미 종료됐거나 종료 가능성이 높은 행사를 진행 중/예정처럼 서술
- 근거에 없는 날짜·장소·가격·프로그램을 단정적으로 서술
- 근거가 칼럼/회고인데 현재 소식처럼 포장

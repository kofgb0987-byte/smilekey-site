...
export async function saveSummary(item) {
  ...
  // 상한 2000 — 199였을 때 옛 글 id가 잘려 사이트맵에서 123개 누락됐던 사고(2026-08-21)
  await redis.ltrim(SUMMARY_LIST_KEY, 0, 1999);
  return true;
}
...
// 전체 요약 id — 목록 list는 ltrim으로 잘리지만 중복방지 set은 전량 보존됨 (사이트맵용)
export async function listAllSummaryIds() {
  return (await redis.smembers(SUMMARY_IDS_SET_KEY)) || [];
}
...
export async function saveDaeguPost(post) {
  ...
  const toSave = { ...post };
  for (const k of ["sections", "tags", "sources", "images"]) {
  ...
  await redis.ltrim(DAEGU_LIST_KEY, 0, 1999);
  return true;
}
...
// 전체 대구 소식 id — 사이트맵용 (요약 쪽과 동일한 이유로 set 기준)
export async function listAllDaeguIds() {
  return (await redis.smembers(DAEGU_IDS_SET_KEY)) || [];
}
...
// 일별 버킷(KST) — 주간 인기글 집계용, 8일 후 자동 소멸
const daeguViewsDayKey = (offsetDays = 0) => {
  const d = new Date(Date.now() + 9 * 60 * 60 * 1000 - offsetDays * 24 * 60 * 60 * 1000)
    .toISOString()
    .slice(0, 10)
    .replace(/-/g, "");
  return `smilekey:daegu:views:d:${d}`;
};

export async function incrDaeguView(id) {
  const dayKey = daeguViewsDayKey();
  const [score] = await Promise.all([
    redis.zincrby(DAEGU_VIEWS_KEY, 1, id),
    redis.zincrby(dayKey, 1, id).then(() => redis.expire(dayKey, 8 * 24 * 60 * 60)),
  ]);
...
// 최근 7일 조회수 상위 n개 — 일별 버킷 합산 (주간 인기글)
export async function topDaeguViewIdsWeekly(n = 3, days = 7) {
  const totals = new Map();
  for (let i = 0; i < days; i++) {
    const flat = await redis.zrange(daeguViewsDayKey(i), 0, -1, { withScores: true });
    for (let j = 0; j + 1 < flat.length; j += 2) {
      const id = String(flat[j]);
      totals.set(id, (totals.get(id) || 0) + (Number(flat[j + 1]) || 0));
    }
  }
...
export async function getDaeguCommentCountMap(ids = []) {
  const counts = await Promise.all(ids.map((id) => redis.llen(daeguCommentsKey(id))));
  const map = {};
  ids.forEach((id, i) => {
    map[id] = Number(counts[i]) || 0;
  });
  return map;
}
...
// 링크 400여 개를 한 건씩 SISMEMBER 왕복하면(Vercel→터널→집 Redis, ~250ms/회) 100초가 걸려
// 후보 공급이 함수 한도(120s)에 닿는다(2026-09-08 실측 108s). SMISMEMBER로 청크당 1회 왕복.
// Redis 7.4(집 WSL)에서 지원 확인. 혹시 명령이 거부되면 옛 순차 경로로 내려간다.
const SEEN_CHUNK = 100;

export async function filterUnseenLinks(links = []) {
  if (!links.length) return [];
  try {
    const out = [];
    for (let i = 0; i < links.length; i += SEEN_CHUNK) {
      const chunk = links.slice(i, i + SEEN_CHUNK);
      const flags = await redis.smismember(DAEGU_SEEN_KEY, chunk);
      if (!Array.isArray(flags) || flags.length !== chunk.length) throw new Error("smismember 응답 형식 이상");
      chunk.forEach((link, j) => {
        if (!flags[j]) out.push(link);
      });
    }
    return out;
  } catch (e) {
    console.error("filterUnseenLinks smismember 실패, 순차 폴백:", e.message);
    const out = [];
